...
void dag_cycle()
{
        dis[1]=0 ;                                              // initialize distance of source = 0 
        for(auto u:topological)                // the vector that contains nodes according to the topological sort 
        { 
                for(auto i:adj[u]) 
                {
                        int v = i.first ; 
                        int w = i.second ;
                        if(dis[u]==infLL) continue;  
                        if(dis[u]+w < dis[v])
                        {
                                dis[v] = dis[u]+w ; 
                        }
        ...
        for(int i=1;i<=n;i++) cout << dis[i] << " " ; cout el ; 
}
